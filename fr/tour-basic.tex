...
\item[\texttt{date}] La date et l'heure à laquelle le \textit{changeset}
  a été créé, ainsi que le \textit{timezone} dans laquelle il a été créé. %%%TODO: Translate 'timezone' properly
  (La date et l'heure sont locales à cette \textit{timezone}, elles indiquent
  donc quelle date et quelle il était pour la personne qui a créé ce %%%TODO: je suppose (quelle "heure")
  \textit{changeset}.)
...
our faire un enregistrement d'un \textit{changeset} pour une quelquonque %%%TODO: our : "pour" ou "ou"
...
Si le résumé affiché par \hgcmd{log} est utile si vous savez déjà ce %%%TODO: je pense que le premier "si" est de trop
...
cherchez à déterminer si ce \textit{changeset} est bien celui que vous%%%TODO: les propositions sont mal construites : après un "si...." il faut une proposition sans "si... donc ici : "si ... recherchez", ben quoi ?
...
Mercurial a une manière approche directe et cohérente pour interpréter %%%TODO: une manière d'approche ?
...
\item Chaque option a un nom complet. Par exemple, comme nous l'avons déjà
      vu, la command e\hgcmd{log} accepte l'option \hgopt{log}{--rev}.%%%TODO: commande ou command e\hgcmd...?
...
La plupart des commandes qui affichent une quelquonque sortie à l'écran,
...
Pour définir un nom d'utilisateur, utiliser votre éditeur de texte favori
...
L'éditor que la commande \hgcmd{commit} déclenche ne contiendra
...
Si vous exécuter la commande \hgcmd{commit} sans aucun arguments, elle
...
identique à celle du \hgcmd{log}, maios qui n'affiche que la dernière
révision du dépôt.
...
\dirname{my-hello}. Etudions comment propager cette modification
dans d'autres dépôts.

\subsection{Récupérer les modifications d'autres dépôts}
\label{sec:tour:pull}

Pour commencer, construisons un clône de notre dépôt \dirname{hello} 
qui ne contiendra pas le changement que nous venons d'effectuer. Nous
l'appelerons notre dépôt temporaire \dirname{hello-pull}.
...
Apporter les modifications rapatriées dans un dépôt sera résume donc
...
\hgopt{pull}{-u}, vous pouvez constater qu'elle a afficher un rappel assez
...
Pour découvrir quelle révision de l'espace de travail en est, utiliser
